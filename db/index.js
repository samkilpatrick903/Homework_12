...
async function viewEmpByDep() {
  ...
  const depQuery = await connection
    .promise()
    .query(
      `SELECT id FROM department WHERE department.dep_name = ?`,
      depName.dep_name
    );
  const query = await connection.promise().query(
    `SELECT department.id,
        ...
        JOIN employee ON employee_role.id = employee.role_id WHERE department.id = ?`,
    depQuery[0][0].id
  );
  return query;
}
...
async function addDept() {
  ...
  await connection
    .promise()
    .query(`INSERT INTO department (dep_name) VALUE (?)`, depInfo.dep_name);
...
async function addRole() {
  const roleInfo = await inquirer.prompt([
    ...
    },
    {
      name: "department_id",
      message:
        "What department is the new role under? Please input department id.",
      type: "number",
    },
  ]);
  await connection
...
async function addEmp() {
  ...
  await connection
    .promise()
    .query(
      `INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES (?, ?, ?, 1)`,
      [empInfo.first_name, empInfo.last_name, empInfo.role]
    );
  ...
  const updateInfo = await inquirer.prompt([
    {
      name: "first_name",
      message:
        "Whose role would you like to update? Please input first name with first letter capatalized.",
      type: "input",
    },
    {
      name: "last_name",
    ...
    },
    {
      name: "update_role",
      message:
        "What would you like to update their role to? Please input their role id.",
      type: "number",
    },
  ]);
  await connection
